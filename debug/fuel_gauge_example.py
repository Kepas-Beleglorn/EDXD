...
class FuelGauge(wx.Panel):
    ...
    def SetShowScale(self, show):
        ...
        if self._show_scale:
            self.SetMinSize((200, 70))
        else:
            self.SetMinSize((200, 40))
        self.Refresh(False)
        self.Layout()
    ...
    def OnPaint(self, event):
        ...
        if frac * 100.0 <= self._warning_threshold:
            text_col = wx.Colour(255, 255, 255)
        else:
    ...
    def _draw_scale(self, gc, bar_rect):
        ...
        gc.SetFont(font)
        gc.SetTextForeground(wx.Colour(200, 200, 200))
...
class DemoFrame(wx.Frame):
    def __init__(self):
        super().__init__(None, title="Fuel Gauge Demo", size=(400, 300))
        panel = wx.Panel(self)
        vbox = wx.BoxSizer(wx.VERTICAL)

        # Variant 1: just the multi-colour bar
        self.gauge_simple = FuelGauge(panel, show_scale=False)

        # Variant 2: bar with scale/ticks
        self.gauge_scale = FuelGauge(panel, show_scale=True)
        ...
        self.threshold_spin = wx.SpinCtrl(panel, min=1, max=99, initial=10)

        vbox.Add(self.gauge_simple, 0, wx.EXPAND | wx.ALL, 10)
        vbox.Add(
            self.gauge_scale,
            0,
        ...
        hbox.Add(
            wx.StaticText(panel, label="Fuel level:"),
            0,
        ...
        hbox.Add(
            wx.StaticText(panel, label="Warning <="),
            0,
            wx.ALIGN_CENTER_VERTICAL | wx.RIGHT,
            5,
        )
        ...
        self.threshold_spin.Bind(wx.EVT_SPINCTRL, self.OnThresholdChange)

    def OnSlider(self, event):
        value = event.GetInt()
        self.gauge_simple.SetLevel(value)
        self.gauge_scale.SetLevel(value)

    def OnThresholdChange(self, event):
        val = self.threshold_spin.GetValue()
        self.gauge_simple.SetWarningThreshold(val)
        self.gauge_scale.SetWarningThreshold(val)
