...
class DemoFrame(wx.Frame):
    def __init__(self):
        super().__init__(None, title="FSD state demo", size=wx.Size(400, 400))
        panel = wx.Panel(self)
        sizer = wx.BoxSizer(wx.VERTICAL)

        # FSD indicator: make sure parent is the same panel that the sizer will be attached to
        self.fsd_indicator = FSDIndicator(parent=panel, size=wx.Size(200, 100))
        self.fsd_indicator.set_text("FSD STATUS")

        sizer.Add(self.fsd_indicator, 0, wx.CENTER, 5)

        # Buttons to change state (created with parent=panel)
        btn_off = wx.Button(panel, label=f"FSD {self.fsd_indicator.STATE_OFF}")
        btn_off.Bind(wx.EVT_BUTTON, lambda evt: self.fsd_indicator.set_state(self.fsd_indicator.STATE_OFF))
        sizer.Add(btn_off, 0, wx.EXPAND | wx.ALL, 5)

        btn_charging = wx.Button(panel, label=f"FSD {self.fsd_indicator.STATE_CHARGING}")
        btn_charging.Bind(wx.EVT_BUTTON, lambda evt: self.fsd_indicator.set_state(self.fsd_indicator.STATE_CHARGING))
        sizer.Add(btn_charging, 0, wx.EXPAND | wx.ALL, 5)

        btn_super = wx.Button(panel, label=f"FSD {self.fsd_indicator.STATE_SUPERCHARGED}")
        btn_super.Bind(wx.EVT_BUTTON, lambda evt: self.fsd_indicator.set_state(self.fsd_indicator.STATE_SUPERCHARGED))
        sizer.Add(btn_super, 0, wx.EXPAND | wx.ALL, 5)
